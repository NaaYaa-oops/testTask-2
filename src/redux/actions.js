    ...
    return dispatch => {
        (async () => {
            await axios.post(`${API_URL}posts`, post)
                .then(r => dispatch(
                    {type: CREATE_POST, payload: r.data}))
        })()
    }
}
...
export function getCurrentPostComments(id) {
    return dispatch => {
        (async () => {
            await axios.get(`${API_URL}posts/${id}?_embed=comments`)
                .then(r => dispatch(
    ...
    return dispatch => {
        (async () => {
            await axios.put(`${API_URL}posts/${id}`, updatedPost)
                .then(r => dispatch(
                    {type: UPDATE_POST, payload: r.data}))
        })()
    }
}
...
export function addCommentToCurrentPost(postId, body) {
    return dispatch => {
        (async () => {
            await axios.post(`${API_URL}comments`, {postId, body})
